...
public class CRC16Utils {
    ...
    /**
     *
     * @param crc_poly CRC校验时初值
     * @param message 校验值
     * @return
     */
    private static int do_crc(int crc_poly, byte[] message) {
        int crc_16 = crc_poly;
        for (int i = 0; i < message.length; i++) {
//            crc_reg = (crc_reg >> 8) ^ crc16_ccitt_table[(crc_reg ^ message[i]) & 0xff];

            crc_16 = crc16_updateByte(message[i],crc_16);
        }
    ...
    private static int crc16_updateByte(byte data,int crc16){
        byte tmp;
        tmp = (byte)((crc16 >> 12) & 0x0f);
        crc16 <<= 4;
        System.out.println("(data >> 4) ^ tmp = "+((data >> 4) ^ tmp));
        crc16 ^= crc16_ccitt_table[(data >> 4) ^ tmp];
        tmp = (byte)((crc16 >> 12) & 0x0f);
        crc16 <<= 4;
        crc16 ^= crc16_ccitt_table[(data  & 0x0f) ^ tmp];
        return crc16;
    }
...
//        int crc_reg = 0x0000;
        int crc_poly = 0x1021;
        return do_crc(crc_poly, message);
    }
    /**
     * db44检验方法
     *
